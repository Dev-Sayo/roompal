...
const initPropertyListings = async () => {
  ...
  if (!propertyContainer) {
    ...
    for (const container of containers) {
      if (
        container.querySelector(".bg-\\[\\#e9e0cf\\]") ||
        container.id === "property-listings"
      ) {
  ...
  if (!propertyContainer) {
    console.error(
      "Property container not found! Available containers:",
  ...
  try {
    ...
    container.innerHTML = `
      <div class="text-center py-10">
        ...
        <button onclick="location.reload()" class="w-full cursor-pointer px-4 py-2 bg-[#223448] text-white rounded-full">
          Retry
        </button>
      </div>
    `;
...
const renderPagination = (container, pagination, filters) => {
  ...
  paginationContainer.className =
    "pagination-container w-full flex justify-center items-center gap-4 mt-8";
